...
int8_t mpu6050_readBytes(uint8_t regAddr, uint8_t length, uint8_t *data) {
	...
	int8_t count = 0;
	if(length > 0) {
		unsigned char buffer[3 + length];
		buffer[0] = MPU6050_ADDR|0;
		buffer[1] = regAddr;
		buffer[2] = MPU6050_ADDR|1;
		USI_TWI_START_Read_Data(buffer, 3 + length);
...
void mpu6050_writeBytes(uint8_t regAddr, uint8_t length, uint8_t* data) {
	if(length > 0) {
		...
		USI_TWI_Start_Transceiver_With_Data(buffer, 2 + length);
		for (uint8_t i = 2; i < length + 2; i++)
		{
			data[i -2] = buffer[i];
		}
...
void mpu6050_writeWord(uint8_t regAddr, uint16_t data) {
	...
	USI_TWI_Start_Transceiver_With_Data(buffer, 4);
	//write data
	//i2c_start(MPU6050_ADDR | I2C_WRITE);
	//i2c_write(regAddr); //reg
...
void mpu6050_getConvData(double* axg, double* ayg, double* azg, double* gxds, double* gyds, double* gzds) {
	...
	*gzds = (double)(gz-MPU6050_GZOFFSET)/MPU6050_GZGAIN;
	#else
    *axg = (double)(ax)/(double)MPU6050_AGAIN;
    *ayg = (double)(ay)/(double)MPU6050_AGAIN;
    *azg = (double)(az)/(double)MPU6050_AGAIN;
    *gxds = (double)(gx)/(double)MPU6050_GGAIN;
	*gyds = (double)(gy)/(double)MPU6050_GGAIN;
	*gzds = (double)(gz)/(double)MPU6050_GGAIN;
	#endif
}
